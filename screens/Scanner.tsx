...
export const Scanner: React.FC<ScannerProps> = ({ events, members, attendance, groups = [], activeFoundation, onRefresh, onLogout }) => {
  ...
  // ---------------------------------------------------------------------------
  // Dukungan scanner USB / Bluetooth (HID keyboard-wedge)
  // ---------------------------------------------------------------------------
  // Scanner barcode/QR USB & Bluetooth berperilaku seperti keyboard: mengetik
  // isi kode dengan cepat lalu menekan Enter. Kita kumpulkan ketikan beruntun
  // dan saat Enter (atau jeda) ditekan, proses sebagai hasil scan. QR anggota
  // berisi member.id sehingga langsung kompatibel dengan processAttendance().
  const hidBufferRef = useRef('');
  const hidLastTimeRef = useRef(0);
  const hidTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    const SCAN_CHAR_GAP_MS = 50;   // antar-karakter scanner < ~50ms (manusia lebih lambat)
    const MIN_CODE_LENGTH = 4;     // abaikan input pendek/keystroke tunggal

    const submitBuffer = () => {
      const code = hidBufferRef.current.trim();
      hidBufferRef.current = '';
      if (code.length >= MIN_CODE_LENGTH && selectedEventId && !pendingMember && !isProcessing) {
        processAttendance(code);
      }
    };

    const handleKeyDown = (e: KeyboardEvent) => {
      // Jangan ganggu saat user mengetik di input/textarea/select (pencarian manual).
      ...
      const now = Date.now();
      // Reset buffer jika jeda antar tombol terlalu lama (berarti input manusia).
      if (now - hidLastTimeRef.current > SCAN_CHAR_GAP_MS) {
        hidBufferRef.current = '';
      }
      ...
      // Hanya kumpulkan karakter tunggal yang dapat dicetak (huruf/angka/simbol).
      if (e.key.length === 1) {
        hidBufferRef.current += e.key;
        // Fallback bila scanner tidak mengirim Enter: submit setelah jeda singkat.
        if (hidTimerRef.current) clearTimeout(hidTimerRef.current);
        hidTimerRef.current = setTimeout(submitBuffer, 120);
      }
    };
  ...
  }, [selectedEventId, pendingMember, isProcessing, selectedSessionId, members, attendance]);
                ...
                <div className="flex-1 flex flex-col bg-slate-950">
                    {isCameraActive ? (
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-950 text-white pb-32">
                            ...
                            <p className="text-sm text-white/40 mb-5 max-w-[260px]">Silakan pilih acara aktif dan tekan tombol di bawah untuk mengaktifkan kamera.</p>
                            <div className="mb-10 inline-flex items-center gap-2 px-4 py-2 bg-green-500/15 text-green-400 rounded-full text-[10px] font-black uppercase tracking-wider ring-1 ring-green-500/30">
                                <Keyboard size={14} /> Scanner USB / Bluetooth aktif
                            </div>
                            ...
                            <div className="flex-1 flex flex-col">
                                {isCameraActive ? (
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center animate-in fade-in">
                                        ...
                                        <p className="text-sm text-gray-500 mb-4 max-w-[300px]">Arahkan kamera ke QR Code anggota atau gunakan pencarian manual untuk memproses absensi.</p>
                                        <div className="mb-10 inline-flex items-center gap-2 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-full text-[11px] font-black uppercase tracking-wider ring-1 ring-green-200 dark:ring-green-800">
                                            <Keyboard size={16} /> Scanner USB / Bluetooth aktif — langsung scan
                                        </div>
