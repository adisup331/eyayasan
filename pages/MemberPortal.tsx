...
export const MemberPortal: React.FC<MemberPortalProps> = ({
  ...
  const upcomingEvents = useMemo(() => {
    return events
      .filter((e) => {
        ...
        const isInvited = attendance.some(
          (a) => a.event_id === e.id && a.member_id === currentUser.id,
        );
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) {
      setMsg({ text: "Password minimal 6 karakter.", type: "error" });
      return;
    }
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-between text-white relative z-10">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <h2 className="text-sm font-black leading-tight tracking-tight uppercase">
                        {orgName}
                      </h2>
                ...
                <div className="flex flex-col items-center justify-center py-2">
                  ...
                  <div className="mt-4 text-center">
                    <p className="text-xl font-black truncate leading-none mb-1 drop-shadow-md uppercase tracking-tight">
                      {currentUser.full_name}
                    </p>
                    <p className="text-[9px] text-slate-400 font-mono tracking-widest opacity-80">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 p-6 rounded-[2rem] bg-gradient-to-br from-primary-600 to-primary-700 text-white shadow-xl shadow-primary-600/20 relative overflow-hidden group">
                ...
                <div className="relative z-10 flex justify-between items-center">
                  <div>
                    <p className="text-xs font-bold text-primary-100 uppercase tracking-widest mb-1">
                      Persentase Hadir
                    </p>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {upcomingEvents.map((event) => (
                  <div
                    key={event.id}
                    className="group relative flex gap-5 p-5 rounded-[2rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:shadow-xl transition-all hover:-translate-y-1"
                  >
            ...
            {historyList.length > 0 ? (
              <div className="space-y-0 border rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden divide-y divide-slate-100 dark:divide-slate-800 shadow-sm">
                {historyList.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 flex justify-between items-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition"
                  >
                    <div>
                      ...
                      className={`text-[10px] px-2.5 py-1 rounded-full font-bold border ${
                        item.status === "Present"
                          ...
                          : item.status === "Present Late"
                            ...
                            : item.status === "izin_telat"
                              ? "bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-950 dark:text-indigo-400 dark:border-indigo-900"
                              : item.status === "Excused"
                                ? "bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-950 dark:text-slate-400 dark:border-slate-900"
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-8 pb-32">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                <BioItem
                  label="Jenis Kelamin"
                  value={currentUser.gender === "L" ? "Laki-laki" : "Perempuan"}
                  icon={User}
                />
                <BioItem
                  ...
                  value={
                    currentUser.birth_date
                      ? new Date(currentUser.birth_date).toLocaleDateString(
                          "id-ID",
                ...
                <BioItem
                  label="Kelompok"
                  value={(currentUser as any).groups?.name || "-"}
                  icon={Boxes}
                />
                <BioItem label="Organisasi" value={orgName} icon={Building2} />
                <BioItem
                  label="Status Kerja"
                  value={currentUser.employment_status || "Pribumi"}
                  icon={Timer}
                />
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm p-8">
              ...
              {msg && (
                <div
                  className={`p-4 mb-6 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-3 animate-in zoom-in ${msg.type === "success" ? "bg-green-50 text-green-700 border border-green-100" : "bg-red-50 text-red-700 border border-red-100"}`}
                >
                  {msg.type === "success" ? (
              ...
              <form onSubmit={handlePasswordChange} className="space-y-5">
                ...
                <button
                  disabled={loading}
                  className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:opacity-90 active:scale-95 transition-all disabled:opacity-50 shadow-xl dark:shadow-white/10"
                >
