...
export const Dashboard: React.FC<DashboardProps> = ({ members, programs, divisions, events, attendance, organizations, isDarkMode, activeFoundation }) => {
  ...
  const activeWidgets = activeFoundation?.dashboard_config && activeFoundation.dashboard_config.length > 0
    ? activeFoundation.dashboard_config
    : DEFAULT_WIDGETS;

  // --- STATS LOGIC ---
  const totalCost = useMemo(() => programs.reduce((acc, curr) => acc + curr.cost, 0), [programs]);
  const activeProgramsCount = programs.filter(p => p.status === 'In Progress').length;
  const upcomingEventsCount = events.filter(e => e.status === 'Upcoming').length;
  
  // Total Educators (Only in Education/TPQ orgs)
  const totalEducators = useMemo(() => {
      return members.filter(m => {
          const org = organizations.find(o => o.id === m.organization_id);
          // Check org type and if they have a role (assuming educators have roles)
          return org && (org.type === 'Education' || org.type === 'TPQ') && m.role_id;
      }).length;
  }, [members, organizations]);

  // --- EXPIRY LOGIC (3 MONTHS) ---
  const expiryAlerts = useMemo(() => {
    // 1. Check if Education/TPQ exists
    const hasEdu = organizations.some(o => o.type === 'TPQ' || o.type === 'Education');
    if (!hasEdu) return [];

    // 2. Filter logic: End Date <= Today + 90 days (approx 3 months)
    const today = new Date();
    const ninetyDaysLater = new Date();
    ninetyDaysLater.setDate(today.getDate() + 90);

    return members.filter(m => {
        // Must belong to Edu/TPQ
        const org = organizations.find(o => o.id === m.organization_id);
        if (!org || (org.type !== 'TPQ' && org.type !== 'Education')) return false;
  ...
  const attendanceTrend = useMemo(() => {
      ...
      return completedEvents.map(event => {
          ...
          // Use number of records as base, assuming invites were generated
          const totalInvited = records.length || 1; 
          const percentage = Math.round((presentCount / totalInvited) * 100);
  ...
  const renderStats = () => (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {activeWidgets.includes('STATS_MEMBERS') && (
          ...
          {activeWidgets.includes('STATS_EVENTS') && (
            <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-100 dark:border-dark-border flex items-center space-x-4">
            ...
            <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{upcomingEventsCount}</p>
            </div>
            </div>
          )}

          {activeWidgets.includes('STATS_PROGRAMS') && (
            <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-100 dark:border-dark-border flex items-center space-x-4">
            ...
            <div>
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{activeProgramsCount}</p>
            </div>
            </div>
          )}

          {activeWidgets.includes('STATS_BUDGET') && (
            <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-100 dark:border-dark-border flex items-center space-x-4">
          ...
          {activeWidgets.includes('STATS_EDUCATORS') && (
            <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-100 dark:border-dark-border flex items-center space-x-4">
            <div className="p-3 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-lg">
                <GraduationCap size={24} />
            </div>
            <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Total Pendidik</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalEducators}</p>
            </div>
            </div>
          )}
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. Expiry Alerts (3 Months) */}
      {activeWidgets.includes('LIST_EXPIRY') && expiryAlerts.length > 0 && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 animate-in fade-in slide-in-from-top-4 duration-500">
              <h3 className="text-red-800 dark:text-red-300 font-bold flex items-center gap-2 mb-2">
                  <AlertTriangle size={20} /> Peringatan Masa Bakti (Habis dalam 3 Bulan)
              </h3>
              <p className="text-sm text-red-700 dark:text-red-400 mb-3">
                  Terdapat {expiryAlerts.length} Tenaga Pendidik yang masa baktinya akan segera habis.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                  {expiryAlerts.map(m => (
                      <div key={m.id} className="bg-white dark:bg-dark-card p-3 rounded-lg border border-red-100 dark:border-red-900 shadow-sm flex justify-between items-center">
                          <div>
                              <p className="font-semibold text-gray-800 dark:text-white text-sm">{m.full_name}</p>
                              <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                                 <BookOpen size={10} />
                                 {organizations.find(o => o.id === m.organization_id)?.name}
                              </div>
                          </div>
                          <div className={`text-xs font-bold px-2 py-1 rounded ${m.daysLeft <= 0 ? 'bg-red-100 text-red-700' : 'bg-orange-100 text-orange-700'}`}>
                              {m.daysLeft <= 0 ? 'Expired' : `${m.daysLeft} Hari`}
                          </div>
                      </div>
                  ))}
              </div>
      ...
      {/* 3. Program Table */}
      {activeWidgets.includes('TABLE_PROGRAMS') && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {activeWidgets.includes('CHART_BUDGET') && (
         ...
         {activeWidgets.includes('LIST_AGENDA') && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {activeWidgets.includes('CHART_ATTENDANCE') && (
            <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-100 dark:border-dark-border">
                <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
