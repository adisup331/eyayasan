...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  // Tampilkan pesan error yang dikembalikan callback OAuth (?error=...).
  useEffect(() => {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Sukses -> browser diarahkan ke Google, lalu kembali ke /auth/callback.
  };
        ...
        {!isForgot && (
            <>
                <button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-3.5 mb-4 rounded-xl border border-gray-200 dark:border-dark-border bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 font-black text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-all shadow-sm active:scale-95 disabled:opacity-50"
                >
                    {googleLoading ? (
                ...
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">atau email</span>
                    <div className="flex-1 h-px bg-gray-200 dark:bg-dark-border" />
                </div>
            </>
        )}
